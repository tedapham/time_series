...
```{r}
k.smooth.wide <- ksmooth(time(unratensa), unratensa, kernel = c("normal"), bandwidth = 9) 
...
```{r}
get.best.arima(emp.log,4,4,1,1)
```
...
```
The adf and the unit root tests both show evidence for weakly stationary for the autosale data. There's no unit root so we can proceed 
straight to VAR modeling. 

But we plot the two series and run the po test just to make sure.

```{r}
#combine log of unemployment data and total autosale
empnsa <- cbind(emp.training, tnsa.training)
...
emp.pred <- ts(empnsa.pred$fcst$emp.training[,1], st = c(2016,1), fr=12)
...
##Compare the 1 year forecast for unemployment produced by the VAR and SARIMA models, examining both the accuracy AND variance of the forecast. Do you think the addition of the automotive sales data helps? Why or why not?
```{r}
ts.plot(emp.test, window(exp(forecast1$mean), end = c(2017,6)), emp.pred, 
        gpars=list(xlab="year", ylab="Unemployment Rate"), 
