...
hist( x = df$O.failed.binary, main = "Distribution of O-ring failures (Binary)", 
      ylab = "Frequency", xlab = "Failure occurence as Binary (0: No | 1: Yes)", ylim = c(0,20),breaks = 0:2 - 0.5, 
...
```{r}
plot(df$Pressure ~ df$Flight, xlab = "Flight Number", ylab = "Combustion Pressure (psi)", main = "Combustion Pressure from past Flight", pch = 19)
```
...
#### 2.3.1 Multivariate Correlation overview
  ...
  - We notice a couple features from the correlation matrix and scatter matrix which can help guide our analysis:
    ...
    - "Temp" is weakly, positively correlated with "Pressure" (correlation value $0.040$). The weakly upward green curve in the scatter matrix confirms. This suggests in a regression model, the independent variables have an effect on the dependent variable, without strong collinearity among themselves, which is good.
...
legend("topright", c("Mean(overlap with median)", "Regression Line"), col = c("red", "green"), lty = c("dotted","solid"))
```
...
legend("topleft", c("Mean", "Median", "Regression Line"), col = c("red", "blue","green"), lty = c("dotted","dotted","solid"))
```
...
### 2.4 Note on Random Sampling
  ...
  - Using the EDA above, we also developed some intuition about sampling of the data. Since some variables (such as pressure) are obviously manipulated and we are interested in causal inference, we try to understand the observations as an experiment dataset. We may assume that our participants are the 23 launches, our independent variables of interest are temperature and pressure. We are trying to associate exposure to temperature and pressue with an increase or decrease in occurence of primary O-ring failure.
...
```
  ...
  - Point estimations of the coefficients in our binomial model agree with that of the author's. We acknowledge that our standard errors are different from the author's and a quasi binomial regression model would have given closer estimates. We provide a residual analysis with plots shortly to address this.
    ...
    - Temperature (-0.098) has a negative sign, suggesting an increase of temperature decreases expected odds of O-ring failure. The estimated odds of O-ring failure changes by `r round(exp(sd(df$Temp)* as.numeric( O.failed.binomial.glm$coefficients[2])),3)` times ,equivalent to decrease by around 50%, with one standard deviation of increase (`r round(sd(df$Temp),3)`F) in temperature , holding pressure constant. 
    
$$OR = \frac{Odds_{T+c}}{Odds_T} = \frac{e^{\beta_0 + \beta_T(T+c) + \beta_SS}}{e^{\beta_0 + \beta_TT + \beta_SS}}= e^{c\beta_T} = e^{7.057*(-0.098)} = 0.50$$
    
    - Pressure (0.008) has a positive sign, suggesting an increase of pressure increases expected odds of O-ring failure. The estimated odds of O-ring failure changes by `r round(exp(sd(df$Pressure)* as.numeric( O.failed.binomial.glm$coefficients[3])),3)` times ,equivalent to increase by around 78%, with one standard deviation of increase (`r round(sd(df$Pressure),3)`psi) in temperature, holding pressure constant. 
...
```
  ...
  - For our binomial logit model, comparing Wald Test results head to head with LRT results:
    ...
    - Temp remain remain insignificant, with Wald p-value `r round(summary(O.failed.binomial.glm)$coefficients[2,4],3)` being larger than LRT p-value `r LR.binomial.temp.pval`
...
```
  ...
  - Although statistical evidence was not strong enough to support pressure as a plausible explanatory variable, domain knowledge and small sample should not be dismissed in the first place and thus we disagree with the author's decision. 
    ...
    - First, the mechanics of a working O-ring depends on its elasticity, and the stress-strain curve in classic physics explains the relationship between stress(analogous to pressure) and strain(analogous to deformation) quite clearly. Once stress applied on material passes a certina yeild stress, O-ring material can loose its elasticity and deformation becomes permanent. The O-ring can either become plastic or fracture entirely, either of which can cause blowby and eventually O-ring failure.
...
large.dataset = replicate(nBoot, sample_n(data.frame(df$Temp,df$O.failed.binomial), 23, replace=TRUE))

# Update Binomial by simulating the probability from sampled temperature
for (i in 1:nBoot){
  dfi = data.frame(prob = large.dataset[,i]$df.O.failed.binomial, Temp = large.dataset[,i]$df.Temp)
  ...
  # Update
  large.dataset[,i]$df.O.failed.binomial = updateProb
}
...
print("90% CI at 31 F is")
bootstrap90(31,large.dataset,nBoot)[1:2]
...
- Bootstrap provides a tighter confidence intervals than the wald method. This is based roughly on the law of large numbers or the assumption that with enough data we can build an empirical distribution that can be a good approximation of the true distribution to approximate the true distribution.
