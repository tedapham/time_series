...
# See other .pdf from Professor Yau
```
...
stargazer(mroz.lm, mroz.glm, summary = FALSE, type = "text")
```
...
plot(x = seq(from = 30, to = 55), predicted.values.lm.k3, type = "l")

#par(mfrow = c(2,1))
plot(x = seq(from = 30, to = 55), pi.hat.k0, type = "l")
plot(x = seq(from = 30, to = 55), pi.hat.k3, type = "l")
...
mroz.glm.noage <- glm(lfp ~ k5 + k618  + wc + hc + lwg + inc,
                family = 'binomial', data = Mroz)

Anova(mroz.glm, test = "LR") #### Individual coefficients test

anova(mroz.glm, mroz.glm.noage, test = "Chisq") ## Compare this to LRT test.
```
