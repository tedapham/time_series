...
hist(df$Sat, breaks = 0:16 - 0.5
    ...
    ,xlab = "# of 'satellite' males in vicinity"
    )
...
```
- The most polular female horseshoe crab's color is 2, followed by colors 3,4, and 1. 
...
```
- For spine variable, the most common value is 3, followed by 1 and 2.
...
```
- From the scatterplot, we observe that width and weight express a strong linear relationship, leading to similar trend in the bivariate relationship of each of these
2 variables with another variable. The number of sattelite male seem to increase with weight and also width. Width and weight seem to have a negative linear relationship
with color and Spine. We investigate the bivariate relationships further.
### Weight & Width
```{r}
plot(df$Width
     ,df$Weight
...
line.fit = lm(Weight~Width, data=df)
#abline(lm(df$Width ~ df$Weight), col = "gray")
abline(line.fit,col='gray')
legend("bottomright", c("Regression Line")
       , col = c("gray"), lty = c("dotted"))
```
```{r}
summary(line.fit)
```
- The linear fit for weight as the response and width as the explantory variable results in statistically significant coefficients with a large R_squared of 0.78. 
Our logic in choosing the weight as the response variable is that weight is more likely resulted from size like how weight relate to a person height (Need WORK)
### Color & Width
```{r}
par(mfrow = c(1, 2))
...
- The physical size (width and weight) seems to decrease with the ordinal color value, indicating that color can be an important factor in our Poisson model.
...
- There seem to be no clear boundary to distinguish between colors 1,3, and 4. Color 2 crabs size tend to have higher variance than other, probably due to 
the number of color 2 being the highest.
...
```{r}
mod.poi <- glm(formula = Sat~., family = poisson(link='log'),data = df)
summary(mod.poi)
```
